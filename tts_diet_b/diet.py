...
import ticdat
from tts_diet_b.tooltips import input_schema_tooltips, solution_schema_tooltips

# ------------------------ define the input schema --------------------------------
# There are three input tables, with 4 primary key fields and 4 data fields.
input_schema = ticdat.PanDatFactory (
...
def solve(dat):
    ...
    # instead of using any pandas.DataFrame logic, we're just going to use dict-of-dict table representation internally
    dat = input_schema.copy_to_tic_dat(dat)
    ...
    sln_ticdat_schema = solution_schema.clone(clone_factory=ticdat.TicDatFactory)
    if mdl.status == gp.GRB.OPTIMAL:
        sln = sln_ticdat_schema.TicDat()
        ...
        return sln_ticdat_schema.copy_to_pandas(sln, reset_index=True)
# ---------------------------------------------------------------------------------
